...
// .env file or a Railway variable) make the header value invalid. Strip those
// characters before the key is ever used.
...
app.post("/api/analyze", async (req, res) => {
  const problemDescription = (req.body?.problem ?? "").toString().trim();
  ...
  if (!apiKey) {
    return res.status(500).json({
      error:
        "The server is not configured: set the GEMINI_API_KEY environment variable."
    });
  }
  ...
  try {
    ...
    if (!text) {
      return res.status(502).json({ error: "No response from the reasoning engine." });
    }
    ...
    if (e instanceof SyntaxError) {
      return res.status(502).json({ error: "Invalid response format from the reasoning engine." });
    }
    return res
      .status(502)
      .json({ error: `The reasoning engine could not be reached: ${raw}` });
  }
});

// Serve the built frontend (produced by `vite build`).
...
app.listen(port, () => {
  console.log(`DeepResolve server listening on port ${port}`);
});
